...
export function Modal({
  ...
  return createPortal(
    <div
      className="mb-fade-in fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 backdrop-blur-[2px] sm:items-center sm:p-6"
      onMouseDown={(e) => {
    ...
    >
      <div
        ...
        aria-modal="true"
        className={cn(
          'mb-sheet-up sm:mb-slide-up flex max-h-[92dvh] w-full flex-col overflow-hidden border border-line bg-surface shadow-pop',
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className={cn(
            'mb-fade-in absolute z-40 mt-1.5 min-w-44 overflow-hidden rounded-xl border border-line bg-surface py-1.5 shadow-pop',
    ...
    <div className="space-y-1 p-2">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2.5">
          <div className="mb-skeleton h-9 w-9 rounded-lg" />
          <div className="flex-1 space-y-2">
            <div className="mb-skeleton h-3.5 w-1/3 rounded" />
            <div className="mb-skeleton h-3 w-1/5 rounded" />
          </div>
        </div>
      ))}
    </div>
