...
import { sessionCookieName, SESSION_ABS_MS, SESSION_TTL_MS } from './lib/session'
...
/** 变更类请求：Origin 校验 + 自定义头（CSRF 防线；SameSite 由 Cookie 属性承担） */
export async function originCheck(c: Context<AppEnv>, next: Next) {
  ...
  if (c.req.header('x-miniblog') !== '1') throw Errors.forbidden('CSRF')
  return next()
}

/** 会话鉴权（AuthCore：读 sessions 表，滑动续期 + 绝对上限） */
...
export function errorHandler(err: unknown, c: Context<AppEnv>) {
  ...
  console.error(`[miniblog] ${requestId} ${c.req.method} ${c.req.path}`, err)
